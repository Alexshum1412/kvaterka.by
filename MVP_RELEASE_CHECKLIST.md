...
- [x] **Re-register the Telegram webhook with its secret** — done 2026-09-25 by `deploy/apply-release.sh` (`{"ok":true,"description":"Webhook was set"}`); an unsigned POST to `/api/telegram/webhook` answers 401 on production. Since DEC-085 the webhook refuses any delivery without `X-Telegram-Bot-Api-Secret-Token`. Kept below for a future domain or token change: Run `TELEGRAM_BOT_TOKEN=… PUBLIC_BASE_URL=https://kvaterka.by npm run telegram:webhook` once on the server after deploying (HOW_TO_UPDATE_THE_SITE.md §2.10); until then phone verification and linking do not work
