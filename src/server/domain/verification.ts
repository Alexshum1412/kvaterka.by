...
export const LEVEL_EXPLANATION: Record<VerificationLevel, string> = {
  ...
  1: 'Аккаунт подтвердил номер телефона, привязав Telegram. Платформа не проверяла документы, удостоверяющие личность — это подтверждение владения номером телефона, а не экспертиза документа.',
  ...
  if (e.kind === 'IDENTITY') {
    return {
      ...
      explanation:
        ...
        'после того, как аккаунт привяжет и подтвердит номер телефона через Telegram.',
    };
  }
