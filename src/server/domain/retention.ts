...
export const RETENTION_CATALOGUE: readonly TablePolicy[] = [
  ...
  },
  {
    ...
    dataClass: 'PSEUDONYMOUS',
    subject: 'user_id',
    purpose: 'Второй фактор сотрудника: секрет TOTP и счётчик неудачных попыток',
    onErasure: 'HARD_DELETE',
    window: { kind: 'TECHNICAL', why: 'Второй фактор не переживает учётную запись' },
    note: 'Секрет хранится в открытом виде: его нельзя хэшировать, потому что он нужен для проверки. Ограничение названо в DEC-054.',
  },
  {
    table: 'two_factor_recovery_code',
    dataClass: 'PSEUDONYMOUS',
    subject: 'user_id',
    purpose: 'Резервные коды на случай потери телефона',
    onErasure: 'HARD_DELETE',
    window: { kind: 'TECHNICAL', why: 'Живут ровно столько, сколько второй фактор' },
