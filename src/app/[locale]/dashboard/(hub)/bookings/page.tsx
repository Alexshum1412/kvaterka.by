...
const TABS = [
  { key: 'REQUESTED', labelKey: 'tabRequested', statuses: ['REQUESTED', 'OFFER_PENDING', 'INQUIRY'] },
  { key: 'CONFIRMED', labelKey: 'tabConfirmed', statuses: ['CONFIRMED'] },
  { key: 'ACTIVE', labelKey: 'tabActive', statuses: ['CHECKED_IN'] },
  { key: 'COMPLETION', labelKey: 'tabCompletion', statuses: ['COMPLETION_PENDING', 'DISPUTED'] },
  {
    key: 'HISTORY',
    labelKey: 'tabHistory',
    statuses: [
...
export default async function LandlordBookingsPage({
  ...
  if (!user) redirect({ href: signInUrl('/dashboard/bookings'), locale });
  ...
  return (
    <div className="container lb">
      ...
      <header className="lb__head">
        ...
        <p className="text-sm muted">
          {counts.get('REQUESTED')! > 0
            ? t('requestsWaiting', { count: counts.get('REQUESTED')! })
            : t('noNewRequests')}
        </p>
      </header>

      <nav className="lb__tabs" aria-label={t('statusAriaLabel')}>
        {TABS.map((tab) => (
          <Link
            key={tab.key}
            href={tab.key === 'REQUESTED' ? '/dashboard/bookings' : `/dashboard/bookings?status=${tab.key}`}
            className="lb__tab"
            aria-current={tab.key === active.key ? 'page' : undefined}
          >
            {t(tab.labelKey)}
            <span className="lb__tabCount">{counts.get(tab.key)}</span>
          </Link>
        ))}
      </nav>

      {awaitingReview.length > 0 && (
        <section className="lb__invite" aria-label={t('reviewsAriaLabel')}>
          <h2 className="lb__inviteTitle">{t('inviteTitle')}</h2>
          <p className="lb__inviteText">{t('inviteText')}</p>
      ...
      <BookingList
        ...
        emptyHint={active.key === 'REQUESTED' ? t('emptyHintRequested') : undefined}
      />
