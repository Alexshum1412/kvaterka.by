...
export default async function RetentionPage() {
  ...
  // does.
  if (!user) redirect({ href: signInUrl('/staff/retention'), locale });
  ...
  function windowLabel(w: Overview['catalogue'][number]['window']): { text: string; tone: string } {
    switch (w.kind) {
      case 'TECHNICAL':
        return { text: t('windowTechnical'), tone: 'verified' };
      case 'PER_ROW':
        return { text: t('windowPerRow', { column: w.column ?? '' }), tone: 'primary' };
      case 'INDEFINITE':
        return { text: t('windowIndefinite'), tone: 'neutral' };
      default:
        return { text: t('windowUndefined', { blockedBy: w.blockedBy ?? '' }), tone: 'warning' };
    }
  }
  ...
  return (
    <StaffShell
      ...
      subtitle={t('subtitle')}
      badges={[
        { label: t('badgeHolds'), count: heldCount },
        { label: t('badgeOverdueReview'), count: overdue, tone: overdue > 0 ? 'warning' : undefined },
        { label: t('badgeStuckPurges'), count: data.stuck.length, tone: data.stuck.length > 0 ? 'danger' : undefined },
      ]}
    >
      <div className="ret">
        {/* The two facts an operator needs before anything else. */}
        <section className="ret__state card">
          <h2 className="ret__h2">{t('stateHeading')}</h2>
              ...
              <div>
                ...
                <p>
                  {data.storage.configured
                    ...
                    : `${data.storage.describe}. ${t('storageNotConfiguredNote')}`}
                </p>
              </div>
            </li>
          ...
          {canRun ? (
            ...
            <p className="ret__muted">{t('runRestrictedToAdmin')}</p>
          )}
        </section>

        {/* Documents by derived state. */}
        <section className="card">
          <h2 className="ret__h2">{t('documentsTitle')}</h2>
          {data.documents.total === 0 ? (
            <p className="ret__muted">{t('documentsEmpty')}</p>
          ) : (
        ...
        {data.stuck.length > 0 && (
          <section className="card ret__stuck">
            <h2 className="ret__h2">
              <Icon name="alert" size={18} /> {t('stuckTitle')}
            </h2>
            <p className="ret__muted">{t('stuckBody')}</p>
            <ul className="ret__list">
              {data.stuck.map((s) => (
                <li key={s.id}>{t('stuckLine', { docType: s.doc_type, when: when(s.purge_started_at) })}</li>
              ))}
            </ul>
          </section>
        )}
        ...
        <section className="card">
          <div className="ret__sectionHead">
            <h2 className="ret__h2">{t('holdsTitle')}</h2>
            <PlaceHold />
          </div>
          <p className="ret__muted">{t('holdsBody')}</p>

          {data.holds.length === 0 ? (
            <p className="ret__muted">{t('holdsEmpty')}</p>
          ) : (
              ...
              {data.holds.map((h) => (
                <li key={h.id} className="ret__hold">
                  ...
                  <p className="ret__meta">
                    {t('holdMeta', { name: h.placed_by_name, when: when(h.placed_at) })}
                    {h.review_overdue && <span className="ret__overdue">{t('holdOverdueSuffix')}</span>}
                  </p>
        ...
        {/* Runs. */}
        <section className="card">
          <h2 className="ret__h2">{t('runsTitle')}</h2>
          {data.runs.length === 0 ? (
            <p className="ret__muted">{t('runsEmpty')}</p>
          ) : (
              ...
              <table className="ret__table">
                <thead>
                  <tr>
                    ...
                    <th>{t('tableStatus')}</th>
                    <th>{t('tableProcessed')}</th>
                    <th>{t('tableSkipped')}</th>
                    <th>{t('tableFailed')}</th>
                  </tr>
                </thead>
                <tbody>
        ...
        {/* The catalogue. */}
        <section className="card">
          <h2 className="ret__h2">{t('catalogueTitle')}</h2>
          <p className="ret__muted">{t('catalogueBody1')}</p>
          <p className="ret__muted">{t('catalogueBody2', { deps: OPEN_LEGAL_DEPENDENCIES.join(', ') })}</p>
                  ...
                  return (
                    <tr key={p.table}>
                      <td>
                        ...
                        {p.appendOnly && <span className="badge badge--neutral">{t('appendOnlyBadge')}</span>}
                        {p.enforced && <span className="badge badge--verified">{t('enforcedBadge')}</span>}
