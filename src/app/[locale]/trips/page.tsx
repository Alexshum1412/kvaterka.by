...
const GROUPS = [
  { key: 'action', statuses: ['COMPLETION_PENDING', 'DISPUTED'] },
  { key: 'waiting', statuses: ['REQUESTED', 'OFFER_PENDING', 'INQUIRY'] },
  { key: 'upcoming', statuses: ['CONFIRMED'] },
  { key: 'active', statuses: ['CHECKED_IN'] },
  {
    key: 'history',
    statuses: [
      'COMPLETED',
...
export default async function TripsPage() {
  ...
  if (!user) redirect({ href: signInUrl('/trips'), locale });
  ...
  // does — asserted once, here, rather than at every later access.
  ...
  const awaitingReview = await services.reviews.pending(userId, 3);
  ...
  const byGroup = GROUPS.map((g) => ({
    ...
    hint: g.key === 'action' ? t('groups.action.hint') : null,
    ...
    <div className="container tp">
      <header className="tp__head">
        ...
        <p className="text-sm muted">
          {total === 0
            ? t('subtitleEmpty')
            : `${t('subtitleCount', { count: total })}${
                needsAction > 0 ? ` · ${t('needsAttention', { count: needsAction })}` : ''
              }`}
        </p>
      </header>

      <nav className="tp__links" aria-label={t('navAria')}>
      ...
      {awaitingReview.length > 0 && (
        <section className="tp__invite" aria-label={t('inviteAria')}>
          <h2 className="tp__inviteTitle">{t('inviteTitle')}</h2>
          <p className="tp__inviteText">{t('inviteText')}</p>
      ...
      {total === 0 ? (
        <div className="tp__empty">
          ...
          <p className="text-sm muted">{t('emptyText')}</p>
