...
/** What each state means for the person reading it, in their own role. Keys mirror the `Booking.explain*` message keys. */
const EXPLAIN_KEY: Record<string, { tenant: string; landlord: string }> = {
  REQUESTED: { tenant: 'explainRequestedTenant', landlord: 'explainRequestedLandlord' },
  CONFIRMED: { tenant: 'explainConfirmedTenant', landlord: 'explainConfirmedLandlord' },
  DECLINED: { tenant: 'explainDeclinedTenant', landlord: 'explainDeclinedLandlord' },
  WITHDRAWN: { tenant: 'explainWithdrawnTenant', landlord: 'explainWithdrawnLandlord' },
  EXPIRED: { tenant: 'explainExpiredTenant', landlord: 'explainExpiredLandlord' },
  CHECKED_IN: { tenant: 'explainCheckedInTenant', landlord: 'explainCheckedInLandlord' },
  NOT_TAKEN_PLACE: { tenant: 'explainNotTakenPlaceTenant', landlord: 'explainNotTakenPlaceLandlord' },
  CANCELLED_BY_TENANT: { tenant: 'explainCancelledByTenantTenant', landlord: 'explainCancelledByTenantLandlord' },
  CANCELLED_BY_LANDLORD: { tenant: 'explainCancelledByLandlordTenant', landlord: 'explainCancelledByLandlordLandlord' },
  DISPUTED: { tenant: 'explainDisputedTenant', landlord: 'explainDisputedLandlord' },
};
...
function formatDay(value: string | null, months: readonly string[]): string | null {
...
const DISPUTE_CATEGORY_KEY: Record<string, string> = {
  LISTING_MISMATCH: 'disputeCategoryListingMismatch',
  ACCESS_PROBLEM: 'disputeCategoryAccessProblem',
  CLEANLINESS: 'disputeCategoryCleanliness',
  PROPERTY_DAMAGE: 'disputeCategoryPropertyDamage',
  PAYMENT_DISAGREEMENT: 'disputeCategoryPaymentDisagreement',
  COMMUNICATION: 'disputeCategoryCommunication',
  SUSPECTED_FRAUD: 'disputeCategorySuspectedFraud',
  CANCELLATION: 'disputeCategoryCancellation',
  NO_SHOW: 'disputeCategoryNoShow',
  OTHER: 'disputeCategoryOther',
};
...
export default async function BookingDetailPage({
  ...
  if (!user) redirect({ href: signInUrl(`/bookings/${id}`), locale });
  ...
  const explainKey = EXPLAIN_KEY[status]?.[isTenant ? 'tenant' : 'landlord'] ?? null;
  const explain = explainKey ? t(explainKey) : null;

  const facts: { icon: IconName; label: string; value: string }[] = [
    { icon: 'calendar', label: t('factCheckIn'), value: formatDate(String(booking.stay_from), months) },
    { icon: 'calendar', label: t('factCheckOut'), value: formatDate(String(booking.stay_to), months) },
    { icon: 'clock', label: t('factDuration'), value: formatNightsLocalized(nights, locale) },
    { icon: 'users', label: t('factGuests'), value: String(booking.guests) },
  ];
    ...
    <div className="container bk">
      <nav className="bk__back">
        <Link href={isTenant ? '/trips' : '/dashboard/bookings'} className="bk__backLink">
          ...
          {isTenant ? t('backToTrips') : t('backToBookingsList')}
        </Link>
      </nav>
      ...
      <header className="bk__head">
        <div className="bk__headMain">
          ...
          <h1 className="bk__title">{listing?.title ?? t('listingFallback')}</h1>
          <p className="bk__meta">
            <Icon name="pin" size={15} />
            {listing?.district ? `${listing.city} · ${listing.district}` : (listing?.city ?? '')}
            {' · '}
            <span className="numeric">{t('referenceLabel', { reference: booking.reference })}</span>
          </p>
        </div>
        {listing?.cover_photo && (
      ...
      {justReviewed && (
        <p className="bk__flash">
          ...
          {query.review === 'published' ? t('flashPublished') : t('flashSaved')}
        </p>
      )}
      ...
      <div className="bk__layout">
        <div className="bk__main">
          ...
          {status === 'COMPLETION_PENDING' && (
            <section className="bk__section bk__section--act">
              <CompletionPanel
                ...
                deadlineLabel={formatDay(booking.completion_deadline_at, months)}
                stayLabel={`${formatDate(String(booking.stay_from), months)} — ${formatDate(String(booking.stay_to), months)}`}
                propertyTitle={listing?.title ?? t('listingFallback')}
                counterpartyName={String(other?.display_name ?? t('counterpartyFallback'))}
              />
            </section>
          )}
              ...
              {canReview ? (
                <div className="bk__invite">
                  <h2 className="bk__inviteTitle">
                    {isTenant ? t('reviewPromptTenant') : t('reviewPromptLandlord')}
                  </h2>
                  <p className="bk__inviteText">{t('inviteText')}</p>
                  ...
                  {booking.review_deadline_at && (
                    <p className="hint">
                      {t('reviewDeadlineHint', { date: formatDay(booking.review_deadline_at, months) ?? '' })}
                    </p>
                  )}
                </div>
              ) : (
                <div className="bk__invite">
                  <h2 className="bk__inviteTitle">{t('rentalCompletedTitle')}</h2>
                  <p className="bk__inviteText">
                    {myReview
                      ? bothReviewsIn && reviewsPublished
                        ? t('reviewsBothPublished')
                        : t('reviewSavedWaiting')
                      : eligibility?.reason === 'WINDOW_CLOSED'
                        ? t('reviewWindowClosed')
                        : t('reviewUnavailable')}
                  </p>
                </div>
              )}
          ...
          {actions.length > 0 && (
            <section className="bk__section">
              <h2 className="bk__h2">{t('actionsHeading')}</h2>
              <BookingActions bookingId={id} actions={actions} />
            </section>
          )}

          <section className="bk__section">
            <h2 className="bk__h2">{t('termsHeading')}</h2>
            ...
            <dl className="bk__rows">
              <div className="bk__row">
                <dt>{t('rentForNights', { nights: formatNightsLocalized(nights, locale) })}</dt>
                <dd>
          ...
          {isLandlord && serviceFee && (
            <section className="bk__section">
              <h2 className="bk__h2">{t('serviceFeeHeading')}</h2>
              <dl className="bk__rows">
                <div className="bk__row">
                  <dt>{t('feeBaseLabel')}</dt>
                  <dd>
                    <Money minor={String(serviceFee.base_minor)} />
                  </dd>
                </div>
                <div className="bk__row">
                  <dt>{t('feeRateLabel')}</dt>
                  <dd className="numeric">{Number(serviceFee.bps) / 100}%</dd>
                </div>
                <div className="bk__row bk__row--total">
                  <dt>{serviceFee.status === 'PAYABLE' ? t('feeDebtLabel') : t('feeAccruedLabel')}</dt>
                  <dd>
                    <strong>
              ...
              <p className="hint">
                {serviceFee.status === 'PAYABLE'
                  ? (() => {
                      const due = formatDay(serviceFee.due_at, months);
                      return due ? t('feePayableHintWithDue', { date: due }) : t('feePayableHint');
                    })()
          ...
          {openCase && (
            <section className="bk__section">
              <h2 className="bk__h2">{t('disputeHeading')}</h2>
              <p className="bk__caseLine">
                <span className="badge badge-warning">
                  {openCase.status === 'RESOLVED' || openCase.status === 'CLOSED' ? t('disputeClosed') : t('disputeInProgress')}
                </span>
                <span className="numeric">{t('referenceLabel', { reference: openCase.reference })}</span>
              </p>
              <p className="text-sm muted bk__caseText">
                {t('disputeCaseText', {
                  category: DISPUTE_CATEGORY_KEY[openCase.category] ? t(DISPUTE_CATEGORY_KEY[openCase.category]!) : openCase.category,
                })}
              </p>
            </section>
          )}
        ...
        <aside className="bk__aside">
          <section className="panel">
            <h2 className="bk__asideH2">{isTenant ? t('hostHeading') : t('tenantHeading')}</h2>
            {other ? (
              <>
                <div className="bk__person">
                  ...
                  <div>
                    ...
                    <p className="text-xs muted">
                      {other.account_kind === 'COMPANY' ? t('companyLabel') : t('individualLabel')}
                    </p>
                  </div>
                </div>
                <ul className="bk__trust">
                  ...
                  <li className="muted">
                    {isTenant
                      ? t('completedDealsCount', { count: Number(other.completed_rentals_as_landlord) })
                      : t('completedRentalsCount', { count: Number(other.completed_rentals_as_tenant) })}
                  </li>
                </ul>
              ...
              <p className="text-sm muted">{t('profileUnavailable')}</p>
            )}
          </section>

          <section className="panel">
            <h2 className="bk__asideH2">{t('chatHeading')}</h2>
            <p className="text-sm muted bk__chatNote">{t('chatHint')}</p>
            {chatId ? (
      ...
      <section className="bk__history">
        <h2 className="bk__h2">{t('historyHeading')}</h2>
        <Timeline bookingId={id} locale={locale} />
      </section>
...
async function Timeline({ bookingId, locale }: { bookingId: string; locale: AppLocale }) {
  const t = await getTranslations('Booking');
  const database = await ready();
  ...
  if (rows.length === 0) return <p className="text-sm muted">{t('noEvents')}</p>;

  const actorLabel = (actor: string | null) =>
    actor === 'TENANT' ? t('actorTenant') : actor === 'LANDLORD' ? t('actorLandlord') : t('actorSystem');
      ...
      {rows.map((e, index) => (
        <li key={index} className="tl__item">
          ...
          <span className="tl__body">
            <strong className="tl__status">{bookingStatusLabel(e.to_status, locale)}</strong>
