...
export async function generateMetadata({
  ...
  try {
    ...
    return {
      title: `${profile.displayName}${t('meta.titleSuffix')}`,
...
function memberSinceDate(value: string, locale: AppLocale): string {
...
export default async function ProfilePage({
  ...
  let trustBandLabel = '';
  try {
    trustBandLabel = t(`trustBand.${profile.trustBand}`);
  } catch {
    trustBandLabel = '';
  }
  ...
  return (
    <div className="container pf">
      <header className="pf__head">
        ...
        <div className="pf__who">
          ...
          <p className="pf__kind">
            {isCompany ? t('kindCompany') : t('kindIndividual')} ·{' '}
            {t('memberSince', { date: memberSinceDate(profile.memberSince, locale) })}
          </p>
          <div className="pf__marks">
            ...
            <span className="badge badge-primary">{trustBandLabel}</span>
          </div>
      ...
      <section className="pf__stats" aria-label={t('statsAria')}>
        {profile.rating !== null && (
          <Stat icon="star" value={profile.rating.toFixed(1)} label={t('reviewsCount', { count: profile.reviewCount })} />
        )}
        {profile.completedRentalsAsLandlord > 0 && (
          <Stat
            icon="home"
            value={String(profile.completedRentalsAsLandlord)}
            label={t('rentalsAsLandlord', { count: profile.completedRentalsAsLandlord })}
          />
        )}
        {profile.completedRentalsAsTenant > 0 && (
          <Stat
            ...
            label={t('rentalsAsTenant', { count: profile.completedRentalsAsTenant })}
          />
        )}
        {profile.activeListings > 0 && (
          <Stat
            ...
            label={t('activeListings', { count: profile.activeListings })}
          />
        )}
        ...
        {profile.responseRate !== null && (
          <Stat icon="message" value={`${Math.round(profile.responseRate * 100)}%`} label={t('responseRate')} />
        )}
      </section>
      ...
      {theirs.length > 0 && (
        <section className="pf__section">
          <h2 className="pf__h2">{t('listingsHeading')}</h2>
      ...
      <section className="pf__section">
        <h2 className="pf__h2">
          {profile.reviewCount > 0 ? t('reviewsHeadingCount', { count: profile.reviewCount }) : t('reviewsHeading')}
        </h2>
        {reviews.rows.length === 0 ? (
          <p className="text-sm muted">{t('noReviews')}</p>
        ) : (
              ...
              <li key={r.id} className="pf__review">
                <div className="pf__reviewHead">
                  ...
                  <span className="pf__reviewRole">
                    {r.author_role === 'TENANT' ? t('reviewAsTenant') : t('reviewAsLandlord')}
                  </span>
                </div>
      ...
      <p className="hint pf__privacy">{t('privacyNote')}</p>
