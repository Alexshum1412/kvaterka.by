...
- Registration confirm = mailed code AND the password chosen at registration (DEC-088). Code alone proves inbox access, not who picked the password (attacker registers victim's address → victim confirms → victim logged into attacker's account). Mailed link never POSTs on load; `/verify-email` shows only what `parseVerifyLink` accepts (attacker-controlled query text must not appear in the site's voice).
- Redirect targets (`next`, cookies, any user value → `redirect`/`Location`/`location.assign`): only via `safeNextPath` (`lib/safe-next.ts`). `startsWith('/') && !startsWith('//')` passes `/\evil.example` and `/<TAB>/evil.example` (browser: `\` = `/`, tab/LF/CR dropped). Non-ASCII gets percent-encoded (Headers throws >U+00FF). DEC-089.
- Phone on an account = proven by Telegram contact share only. Registration never stores a typed phone beside an email. Proven number replaces unproven claim (audit `phone.claim_released`, no number in audit). Telegram chat held only while linked (0026); live on another account → explicit CONFLICT message, never silent, never moved to newer claimant. Account close unlinks Telegram. DEC-089.
- Login lock: read BEFORE password (locked answers all attempts alike, dummy hash) and again after. Never let the answer depend on password correctness. Unique-violation handling in a transaction = `ON CONFLICT DO NOTHING`, not catch-then-query (PG aborts the tx).
