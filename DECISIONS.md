...
**Known and left alone.** (a) Email-code registration has a standing pre-hijack shape: whoever submits an address first chooses the password the emailed code will confirm. This predates the range; the budget bounds it but does not remove it — the real fix is to show the address owner what they are confirming. (b) The global `/api/client-errors` bucket can be exhausted by anyone, which blinds browser-crash telemetry for a minute at a time; accepted, that channel is best-effort and API errors are not behind it. (c) `error_event` sits in the retention catalogue with an invented 90-day window; the catalogue test passes only because its regex uses `\b` around Cyrillic. (d) The e2e suite and the avatar concurrency test could not be run against a real multi-connection PostgreSQL here (no Docker); CI did it. The `real-postgres` job (the whole suite on PostgreSQL 10.23, the parallel avatar replacement test included) passed on the push of these changes. The `e2e` job failed once, on `mobile-menu.spec.ts` — a language pill "44px" high measured 43.99998 at Pixel 7's fractional device scale, an assertion that compared the raw number and passed on the very next push with the same CSS. All size checks in the two mobile specs now round before comparing (commit 8dd4878); no product code was involved. (e) `scripts/contrast.mjs` does not check the dark theme.

**Deploy tooling, found while putting the release on the host.** (1) `npm install --include=dev` on the host answered "up to date in 1s" while `node_modules/next` was 15.5.25 and `package-lock.json` said 15.5.26: the host's install tree carries its own record and `npm install` does not realign it to the lock. Only `npm install --no-save next@15.5.26` moved it — which is exactly the file the RCE fix lives in, so "npm install ran" proves nothing until the installed version is read back. (2) `deploy/kvaterka-build.zip` had backslashes in all 634 entry names: `ZipFile.CreateFromDirectory` in Windows PowerShell 5.1 does not follow the ZIP spec, and Linux `unzip` treats `\` as part of the file name (or warns and converts, depending on the build). `make-deploy-zip.ps1` now writes the entries itself with `/`; the new archive has 625 files, 0 backslashes, and was extracted with `tar` to confirm the layout and `BUILD_ID`. (3) `deploy/apply-release.sh` replaces the manual "migrate, re-register the webhook, back up and extract, restart, check" sequence with one command that runs them in the one safe order (a failed migration stops it before `.next` is touched) and reads the database URL and bot token from the Node selector's own config without printing them. It was exercised against stubs: the happy path, a failing migration, a missing or dirty precondition, a backslash-style zip, `unzip` exiting 1 (warning, tolerated) and 2 (failure), and a config with two apps and no matching key. An automatic permission classifier refused to let the assistant type the migration command into the production terminal on its own; the script exists so that the release is one reviewed command instead of three pasted ones.
...
**Deployed 2026-09-25.** `deploy/apply-release.sh` ran on the host after a dry run that passed every precondition against the real configuration. Migrations applied: **0023, 0024, 0025** ("Applied 3 migration(s); 21 were already in place") — 0023 (the district and property-type search indexes) had been waiting since the hosting outage of DEC-081, so production had been running without it. The Telegram webhook was re-registered (`{"ok":true,"description":"Webhook was set"}`), `.next` was replaced (the previous build is kept as `.next.backup-20260925-1911`, made by hand before the pull, and as a second `.next.backup-<timestamp>` copy the script makes), Passenger restarted. Read back from outside afterwards: `/api/health` answers ok with the new `alerts` field; `/manifest.webmanifest` is 200 (it had always been 404); an unknown URL is a real 404; `/api/client-errors` exists (400 on an empty body); an unsigned POST to `/api/telegram/webhook` is 401; a city landing page carries its own canonical and hreflang ru/be/en on `https://kvaterka.by`; the phone menu offers RU/BE/EN at 44×44 and the signed-out header has no horizontal overflow at 390px. `robots.txt` still disallows everything and the sitemap is empty: `SITE_INDEXABLE` is unset until the launch decision, as intended. Production currently shows no published listings.


## DEC-088 — Registration confirm needs the password, not only the mailed code; the sitemap stops listing empty city pages; search indexing switched on

**Question.** After DEC-087 fixed the mail budget, does the registration flow itself hold up when the person submitting the form is not the person who owns the mailbox? And with the legal questions settled (owner, 2026-09-26: "правовые вопросы решены и платежи будут настроены позже"), what has to be true before search engines are let in?

**Found.** `pending_registration` stores the password hash of whoever submitted the address first, and `confirmRegistration(identifier, code)` created the account and signed the *caller* in on the code alone. The code proves that somebody can read the inbox — nothing more. So: an attacker registers `victim@example.by` with a password of their own; the victim receives a real "confirm your address" mail and follows the link (or types the code); the account is created with the attacker's password and the victim's browser is logged in to it. Anything the victim then puts there — a phone number, a saved search, a booking request — is readable by the attacker by logging in. The mailed link made it worse: the old `/verify-email` page POSTed on load, so the victim did not even have to press anything.

**Fix.** `POST /auth/register/confirm` takes `{ identifier, code, password }`; once the code matches, the password is checked against the stored hash. A wrong password is decided exactly like a wrong code — same 401, same attempt counter written outside the deciding transaction (the load-bearing shape of that function) — so it cannot be guessed past the lockout. The real registrant knows the password; the victim does not and cannot finish. `/verify-email` no longer sends anything on load: it shows the address and asks for the password (show/hide toggle, server message kept on the form); the inline code step on `/login` sends the password it already holds. Second-review findings on that page were also closed: query values are attacker-controlled (anyone can send someone a link), so `lib/verify-link.ts` lets through only what a real link carries — an email or `+375` number, and a 4–10 digit code, single-valued — and everything else falls back to the "no code in the link" block. React never rendered the text as HTML, but "your account is blocked, enter your mailbox password" would have appeared in the site's own voice above a password field.

**Transition.** A tab that loaded the previous bundle before the deploy and submits the code step afterwards sends no password and gets a 400 from the route's zod body; a reload fixes it and the code is valid for an hour. No migration.

**Sitemap.** All six city landing pages were listed unconditionally. With no listings (production today) each is an empty result page — the thin page a crawler files as a soft 404, and the first thing search engines would learn about the site. A city is now listed only when it has at least one published, non-deleted listing, matched case-insensitively exactly as `/search` does (`lower(p.city) = lower($1)`); the query the sitemap already ran for listings now also returns `city`, so it costs nothing extra.

**Verification.** `npm run verify`: 47 files, 1487 tests passing, 1 skipped (was 1467). New negatives: wrong password with a right code, right password with a wrong code, both spending attempts, lockout, and the correct pair still signing in; `parseVerifyLink` refuses markup, prose, look-alike phone numbers, over-long values, bad codes and repeated parameters. Reverting the password check turns the wrong-password tests red; reverting the sitemap change turns the two new city tests red (10 of 12 pass) and restoring it gives 12 of 12. `e2e/registration.spec.ts` drives the flow in a browser. Formatting of `auth-service.ts` was brought to the repo's `printWidth: 110` in its own commit so the behaviour change is 26 lines instead of a screen of re-wrapping.

**Deployed and indexing enabled, 2026-09-26.** CI on `b1b03a7`: verify, e2e (including the new `registration.spec.ts`) and real-postgres all green. `deploy/apply-release.sh` ran after a dry run: "Schema is current — 24 migration(s) already applied" (this release has none), the Telegram webhook was already set, `.next` replaced from a 625-entry zip made by `make-deploy-zip.ps1` (no backslashes, `BUILD_ID` present, `unzip -t` clean on the host), Passenger restarted, `/api/health` ok with `alerts: []`. Read back from outside: `POST /api/auth/register/confirm` without a password answers 422 `VALIDATION_FAILED` on `password` (the route is the new one); `/verify-email?identifier=<prose>&code=123456` renders only the "no code in the link" block in a real browser — the prose reaches the HTML only inside Next's own serialized segment key, never as visible text; a well-formed link renders the password form. Then the owner's go for indexing ("сделай индексацию после того как закончишь") was carried out: `SITE_INDEXABLE=true` added in the cPanel Node.js app's environment (no other variable touched) and saved. Read back: `robots.txt` is `Allow: /` with `/dashboard/`, `/staff/`, `/moderation/`, `/notifications`, `/bookings/`, `/trips`, `/favorites`, `/verify-`, `/password-reset` (and their `/be` and `/en` forms) and `/api/` disallowed, plus `Sitemap: https://kvaterka.by/sitemap.xml`; `/sitemap.xml` lists 11 URLs — the static pages — and, correctly, no city and no listing, because production has no published listing; the home page carries `index, follow`; `/dashboard` still answers `X-Robots-Tag: noindex, nofollow`. **Not done because it is not the assistant's to do:** registering the domain in Google Search Console and Yandex Webmaster and submitting the sitemap (needs the owner's login), and replacing the "document in preparation" text on `/terms` and `/privacy` (needs the lawyer's documents; the owner's statement that the legal questions are resolved is recorded in `MVP_RELEASE_CHECKLIST.md`, but the answers are not yet in `docs/LEGAL.md`, so no LEGAL box was ticked).


## DEC-089 — An audit of the other account flows for DEC-088's kind of bug: six real defects (an open redirect, a phone-squatting lock-out, a Telegram chat that could never be freed, a lock that guessed nothing, a confirm race that 500ed), and a review of the fixes that found three more

**Question.** DEC-088 was "state chosen by the wrong party and later confirmed by the right one". Do the other flows that create or change who an account is (Google sign-in, password reset and other tokens, adding a phone or a Telegram chat, sessions, the registration code itself) have the same shape, or the ordinary takeover, enumeration and token bugs?

**Method.** Four read-only reviewers, one per surface (Google, reset and tokens, identity change and sessions, registration a second time), each required to give file:line evidence and a concrete attacker script, then two skeptics per finding, one trying to refute it from the code and one asking whether an unauthenticated attacker can actually execute it. 46 agents. 15 of the skeptics died on the session limit before answering; their findings (all on the registration surface) were left "unverified" by the script and I judged each myself by reading the code, which is why some appear below as confirmed although the workflow printed them as rejected. Then a second workflow reviewed the fixes: four lenses, 18 agents, including a 506,000-input fuzz of the new redirect check.

**Confirmed and fixed** (each test written first and seen to fail for the stated reason; each fix broken again afterwards and the test seen to fail):

1. **Open redirect through `next`** (found on three surfaces). `startsWith('/') && !startsWith('//')` passes `/\evil.example` and `/<TAB>/evil.example`, which the URL parser reads as `//evil.example`; the login form then called `location.assign` on it. Reachable from `/login?next=…`, `/verify-phone?next=…` and Google start. `lib/safe-next.ts` is now the only check: refuses control characters and backslashes, then asks the URL parser whether the result stays on the origin; non-ASCII is percent-encoded because the value ends in a `Location` header (found by the review of the fix: a character above U+00FF makes `Headers` throw and failed a Google sign-in that had just succeeded). The fuzz found no value leaving the origin, and a sweep of `src` found no other place where user input reaches a redirect.
2. **A phone typed beside an email was stored unproven in `app_user.phone` (UNIQUE)**, so a stranger could register the victim's number and the victim's mandatory Telegram verification then failed with no message. It is no longer stored beside an email; only the Telegram contact share proves a number.
3. **A Telegram chat could never be freed.** `telegram_chat_id` was UNIQUE for ever and unlinking kept the row. Migration `0026` makes it unique only while linked. A chat live on another account now answers an explicit conflict ("send /unlink from this chat") and does not spend the one-time code; it is not moved silently, because a link code is something anybody can mint and hand over. A number proved through Telegram now replaces an unproven claim on another account (audited as `phone.claim_released`, without the number); an account whose only contact is that phone is left alone (`app_user_has_contact`), and a number held by a proven owner gets a conflict message. Found by the review of the fix: the phone-only case, which raised a constraint error nobody saw.
4. **The login lock guessed nothing.** It was read after the password, so once locked a wrong guess still got 401 and only the right one got 429, which stopped no guessing and told the guesser they had won. Now read first (a locked account answers every attempt alike and does not evaluate it, with a dummy hash to keep the cost equal) and again after the password, because the review showed the old post-password read had also been the only thing refusing a correct password that arrived after the lock engaged during a burst of parallel guesses.
5. **`confirmRegistration`'s race branch was dead code on PostgreSQL**: a unique violation aborts the transaction, so the clean-up in the `catch` failed with "current transaction is aborted" and the person got a 500 with the pending row left behind. Now `INSERT … ON CONFLICT DO NOTHING RETURNING id`.
6. **Closing an account left its Telegram link live** (consent to be pinged, withdrawn with the account, and holding the chat against the person's next account). Not from the audit: the review of fix 3 pointed at it and both reviewers called it older than this change; it is a two-line fix in the same transaction, so it went in with a test.

**Judged and deliberately not changed.**
- `POST /auth/register` answers 409 for an existing address and 201 for a new one. That is an existence oracle, and the vague message is moot, but it is a product decision that an existing test states in words, and it is why two further oracles the audit raised are not worth closing: the password-reset timing difference (SMTP delivery inline for known addresses is DEC-070's documented choice, and deferring it delays reset mail by a scheduler tick) and the new fact that a locked known account answers 429 where an unknown one answers 401 (nine requests per probe against a register oracle that costs one). Revisit them together if the 409 is ever changed.
- **Lock-out of a victim** by eight wrong guesses (15 minutes) is inherent to a per-account lock; a per-IP limit of 10 per 15 minutes is the only brake and depends on `X-Real-IP` being overwritten by the proxy, which DEC-087 says is unverified. Not tested against production without the owner's say-so.
- **The lock is a check-then-act count**, so a burst of parallel guesses passes the first look together; the second look bounds what lands during the hashes, not the burst itself. A per-account advisory lock would serialise it; not built.
- **Registration code guessing across parallel confirms** (the attempt counter is written after the deciding transaction): bounded by the per-IP limit, the mail budget per address, the hour the code lives and now also by the password the guesser must present.
- **Session-holder actions without step-up**: closing the account, replacing the Telegram chat or the verified phone. Reachable only with a stolen session; a real hardening, out of this batch.
- **A Telegram deep link minted by an attacker and opened by the victim** still binds the victim's chat to the attacker's account until the victim sends `/unlink` (which now works). Showing the account name in the bot's reply would help but is a change to a reply LEGAL-015 keeps content-free.
- **A closed account keeps its verified phone** (closure is not erasure; LEGAL-003), so its owner cannot verify the same number on a new account. That used to fail silently; it now says the number is tied to another account and to write to support.

**Verification.** `npm run verify`: 48 files, 1509 tests passing, 1 skipped (was 1487). Each of the new tests failed before its fix with the reason named in the commit (the literal "current transaction is aborted", the constraint names `telegram_connection_telegram_chat_id_key` and `app_user_phone_key`, the bot answering "code expired"). Mutation check: removing the encoding, the email guard on the release, the close-account unlink or the post-password lock read each turns its test red. Lint caught one thing on the way (`no-control-regex` in the new helper). Formatting of seven files was brought to the repo's `printWidth` in its own commit first, so the behaviour change stays readable.

**Deployed 2026-09-26.** CI on `d07baf8`: verify, e2e and real-postgres (PostgreSQL 10.23) all green, so migration `0026` had already run on the production engine. `deploy/apply-release.sh` after a dry run: "applying 0026_telegram_chat_release.sql — Applied 1 migration(s); 24 were already in place", the Telegram webhook already set, `.next` replaced from a 625-entry zip (its size on the host matched the local file to the byte), Passenger restarted, site 200, `/api/health` ok with no alerts and the three jobs succeeding after the restart. Read back from outside: `/login?next=` with `/%5Cevil.example/x`, `/%09/evil.example` and `//evil.example` renders `next: /dashboard`; `/dashboard/bookings` is kept; a percent-encoded Cyrillic path is kept encoded; an unsigned POST to the webhook is 401; `robots.txt` is still open (`SITE_INDEXABLE` survived the restart). Deploy note: the host's terminal is a canvas in a background tab, so command output was written to `tmp/*.txt` and read back through cPanel's own file API instead of a screenshot.

**Search Console.** The assistant tried to add the `google-site-verification` TXT record to the `kvaterka.by` zone in cPanel and was refused by the automatic permission check ("DNS / Domain / Cert Changes"); it did not work around it. The owner added the record themselves: `dig`-equivalent lookup shows both TXT records at the apex (the verification token and the untouched SPF), and the Search Console property `sc-domain:kvaterka.by` is open with "data is being processed, retry in about a day". Submitting `sitemap.xml` there and adding the site to Yandex Webmaster are the owner's.
